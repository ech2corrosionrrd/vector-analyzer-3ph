...
import { X, Trash2, FileText, Calendar, Activity, Download } from 'lucide-react';
import type { ArchiveItem } from '../../types/vaf';
...
export function ArchiveModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-modal-fade-in"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden shadow-blue-900/10 animate-zoom-in"
      ...
      >
        <header className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="archive-modal-title" className="text-xl font-bold text-slate-100">Архів вимірювань</h2>
          ...
          {items.length === 0 ? (
            <div className="py-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-600">Збережіть замір, щоб додати його в цей список</p>
              </div>
            </div>
          ) : (
            [...items].sort((a, b) => b.id.localeCompare(a.id)).map((item) => (
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-blue-600/20 text-blue-400">
                    <Activity size={20} />
                  </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                      ...
                      {item.data.objectName && (
                        <span className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded border border-slate-700">
                           Об&apos;єкт: {item.data.objectName}
                        </span>
                      )}
                      {item.data.voltageLevel && (
                        <span className="text-[10px] bg-blue-900/20 text-blue-400 px-1.5 py-0.5 rounded border border-blue-500/20 font-bold">
                           {item.data.voltageLevel} кВ
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2 md:opacity-0 md:group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                  <button
                    type="button"
                    onClick={() => onLoad(item)}
